...
class MainActivity4 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnSaveThu.setOnClickListener {
            if (uri!=null){
                uploadImage(uri!!)
            }
            saveEmployeeData()
        }
        binding.btnAnhThu.setOnClickListener {
  ...
  //Lưu dữ liệu
    private fun saveEmployeeData() {
        ...
        currentUser?.let { user->
            val userId=(user.email?:"").substringBefore("@")
            val emDate= binding.txtDate.text.toString()
            val empTime= binding.txtTime.text.toString()
            val empKhoanThu= binding.edtKhoanThu.text.toString()
            val empLuongThu= binding.edtLuongThu.text.toString()
            val idThu=dbRef.push().key!!
            val employeeDataRef=dbRef.child(userId)
                .child("Nam")
                .child("Thu")
                .child(idThu)
            val employeeThu=employeeThu(idThu,emDate,empTime,empKhoanThu,empLuongThu)
            if (empKhoanThu.isEmpty()){
                binding.edtKhoanThu.error="Vui lòng nhập chi tiết khoản thu"
                return
            }
            if (empLuongThu.isEmpty()){
                binding.edtLuongThu.error="Vui lòng nhập chi tiết khoản thu"
                return
            }
