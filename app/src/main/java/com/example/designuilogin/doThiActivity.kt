...
import com.example.designuilogin.databinding.ActivityDoThiBinding
...
private lateinit var binding: ActivityDoThiBinding
...
class doThiActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityDoThiBinding.inflate(layoutInflater)
        ...
        btnTruyVan.setOnClickListener {
            ...
            if (year.isNotEmpty()) {
                ...
                Toast.makeText(this@doThiActivity, "Vui lòng chọn năm", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        if (!initialNetworkStatus) {
            Toast.makeText(this@doThiActivity, "Mạng yếu hoặc không có kết nối", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun setupChartData(thuList: List<employeeThu>, chiList: List<employeeChi>, yearSelect: String) {
        ...
        val lineEntries = ArrayList<BarEntry>()
        ...
        for ((index, thangDoanhThu) in thangDoanhThuList.withIndex()) {
            ...
            lineEntries.add(BarEntry(index.toFloat()+0.4f, thangDoanhThu.chi))
        }

        val barDataSet = BarDataSet(barEntries, "Thu nhập").apply {
            color = resources.getColor(R.color.green)
        }
        val lineDataSet = BarDataSet(lineEntries, "Chi tiêu").apply {
...
//        val lineData = BarData(lineDataSet as IBarDataSet)
//
//        barData.notifyDataChanged()
//        lineData.notifyDataChanged()
        val barData = BarData(barDataSet, lineDataSet).apply{
            barWidth = 0.4f
        }
        barData.notifyDataChanged()
...
//            setData(lineData)
        ...
        combinedChart.apply {
            ...
            xAxis.valueFormatter =
                IndexAxisValueFormatter(months)  //định dạng trục x theo mảng months
            ...
            legend.verticalAlignment =
                Legend.LegendVerticalAlignment.TOP   //chú thích hiển thị bên trên
            legend.horizontalAlignment =
                Legend.LegendHorizontalAlignment.RIGHT  //chú thích ở bên phải
